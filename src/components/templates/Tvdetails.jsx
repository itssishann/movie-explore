...
import {asyncloadtv} from "../../stores/actions/tvActions"
import Loader from './Loader';
import { removetv } from '../../stores/reducers/tvSlice';
...
function translateHandler() {
  Swal.fire({
    ...
    showConfirmButton: true,
    html:`<p>${info.translations.join(" ")}</p>`
   })
}
  function trailerHandler() {
    if (info.videos === undefined) {
        ...
        Swal.fire({
            title: "Trailer!",
            html: `<iframe className="rounded-lg" width="450" height="320" src="https://www.youtube.com/embed/${info.videos.key}?autoplay=1;" frameborder="0" allow="autoplay" allowfullscreen></iframe>`,
            showCloseButton: true,
            showConfirmButton: true
        });
    }
  ...
  return  info ? (
    <div style={{
      background: `linear-gradient(rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.75)), url(https://image.tmdb.org/t/p/original/${info.detail.backdrop_path})`,
 ...
 {/* Part 2 poster details  */}
    ...
    <div className="content ml-[5%]">
      <h1 className='text-5xl font-black  text-zinc-100'>
        {
          ...
          <span className='pl-2 font-bold text-2xl text-zinc-300'>({info.detail.first_air_date.split("-")[0]})</span>
      </h1>
      ...
      {info.detail.vote_average && (
           ...
           <div className='mt-2  '>
            ...
            <h2 className='text-xl mt-1 text-zinc-300'>
  {info.detail.overview.slice(0, 250)}
  {info.detail.overview.length > 300 && ( // Check if the length is greater than 50
     <span onClick={overviewHandler} className='p-2 text-blue-500 cursor-pointer'>
       more..
    </span>
  )}
</h2>
  ...
  {info.translations.join(" ").length > 100 && ( 
    <span onClick={translateHandler} className=' text-blue-500 cursor-pointer'>and more..</span>
  )}
</h2>
